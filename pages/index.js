...
export default function Home({ days }) {
  ...
  async function handleDelete(id, dataPointId, attribute) {
    ...
    try {
      const response = await fetch(`/api/Days?id=${id}&dataPointId=${dataPointId}&attribute=${attribute}`, {
        method: 'DELETE',
      });
      await response.json();
      refreshData();
  ...
  if (session) {
    ...
    return (
      <CardContainer>
        {days.length === 0 && (
          <>
            <EmptyHeading>{"You don't have any data saved right now"}</EmptyHeading>
            <LottieContainer ref={container} />
          </>
        )}
        <Dashboard weight={latestWeight} height={latestHeight} feastTime={latestFeastTime} />
